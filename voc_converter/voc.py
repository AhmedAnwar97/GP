...
class VOCEgestor(Egestor):
    ...
    def egest(self, image_detections, root):
        ...
        for image_detection in image_detections:
            ...
            shutil.copyfile(image['path'], "%s/%06d.%s" % (images_path, int(image_id), src_extension))

            with open("%s/trainval.txt" % image_sets_path, 'a') as out_image_index_file:
                out_image_index_file.write('%06d\n' % int(image_id))
            ...
            if image['segmented_path'] is not None:
                ...
                shutil.copyfile(image['segmented_path'], "%s/%06d.png" % (segmentations_path, image_id))

            xml_root = ET.Element('annotation')
            add_text_node(xml_root, 'filename', "%06d.%s" % (int(image_id), src_extension))
            ...
            ET.ElementTree(xml_root).write("%s/%06d.xml" % (annotations_path, int(image_id)))
